...
static Module *build_bird()
{
    Module *bird = module_create();
}
...
int main(int argc, char *argv[])
{
    ...
    //     char buffer[256];
    //     sprintf(buffer, "final/hoppybird/hoppybird_frame%03d.ppm", i);
    //     image_write(src, buffer);

    //     image_reset(src);
    // }

    point_print(&rb->position, stdout);
