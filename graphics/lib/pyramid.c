...
Pyramid *pyramid_create(Point *cp, float h, float w, float l)
{
    ...
    Point base[4];
    point_set3D(&base[0], cp->val[0] + dx, y, cp->val[2] + dz);
    point_set3D(&base[1], cp->val[0] + dx, y, cp->val[2] - dz);
    point_set3D(&base[2], cp->val[0] - dx, y, cp->val[2] - dz);
    point_set3D(&base[3], cp->val[0] - dx, y, cp->val[2] + dz);
