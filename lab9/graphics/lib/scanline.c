...
static Edge *makeEdgeRec( Point start, Point end, Image *src, Color *c1, Color *c2)
{
	...
	******/
	...
	float eps = 0.0001;
	edge->dcPerScan.c[0] = ( c2->c[0] - c1->c[0] + eps) / dscan;
	edge->dcPerScan.c[1] = ( c2->c[1] - c1->c[1] + eps ) / dscan;
	edge->dcPerScan.c[2] = ( c2->c[2] - c1->c[2] + eps ) / dscan;
...
static void fillScan( int scan, LinkedList *active, Image *src, DrawState *ds) {
  ...
  while(p1) {
	...
	//   }
	  ...
	  double eps = 0.00001;
	  dcPerCol.c[0] = (c2.c[0] - c1.c[0] + eps) / (endX - startX);
	  dcPerCol.c[1] = (c2.c[1] - c1.c[1] + eps) / (endX - startX);
	  dcPerCol.c[2] = (c2.c[2] - c1.c[2] + eps) / (endX - startX);
	  ...
	  for (int i = startX; i < endX; i++)
	  {
		...
		if (z >= image_getz(src, scan, x))
		{
			...
			{
				tmp.c[0] = (1.0 - (1.0/z)) * ds->color.c[0];
				tmp.c[1] = (1.0 - (1.0/z)) * ds->color.c[1];
				tmp.c[2] = (1.0 - (1.0/z)) * ds->color.c[2];
			}
			else if (ds->shade == ShadeFlat || ds->shade == ShadeConstant)
			{
				color_copy(&tmp, &p1->color);
			}
